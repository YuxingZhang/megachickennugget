...
/* update U' sigma */
void ComputeUpSigma(mat& up_s, double* word_embedding, double& beta, double& l, int& WORD_DIM, int& V){
	double temp = 0;
	for(int i = 0; i < V; i++){
...
/* update eta */
bool UpdateEta(int d, mat& eta_m, mat& eta_s, mat& xi_DK, vec& alpha_D, mat& u_m, mat& a_m, vector<mat>& z, double& gamma, int& N, int& K, double& EPS){
	bool converge = true;
	double temp;
	...
	for(int k = 0; k < K; k++){
		...
		eta_m(d, k) = gamma * dot(u_m.row(k), a_u.row(d)) + N[d] * (2 * alpha_D(d) * lambda(xi_DK(d, k)) - 0.5) + temp;
		eta_m(d, k) *= eta_s(d, k);
	}
